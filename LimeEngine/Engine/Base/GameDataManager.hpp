...
namespace LimeEngine
{
	...
	public:
		...
		GameResourceData LoadResourceData(const std::string gamePath, std::string loadParams) const
		{
		...
		[[nodiscard]] GameResourceRef<Texture2D> CreateTexture2D(const std::string gamePath, std::wstring filePath, TextureType type)
		{
		...
		GameResourceRef<Mesh> LoadMesh(std::string gamePath);
		GameResourceRef<Material> LoadMaterial(std::string gamePath);
		GameResourceRef<Texture2D> LoadTexture2D(std::string gamePath);
