...
namespace LimeEngine
{
	...
	public:
		Matrix(
			...
			float m32,
			float m33) noexcept :
			xmMat(m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33)
		{}
		...
		static void DecomposeTransform(TempVector& location, TempVector& rotation, TempVector& scale, const TempMatrix& mat)
		{
			DirectX::XMMatrixDecompose(&scale, &rotation, &location, mat);
		}
		static void DecomposeTransform(Vector& location, Rotator& rotation, Vector& scale, const TempTransformMatrix& mat)
		{
			TempVector tempLocation{}, tempRotation{}, tempScale{};
			DecomposeTransform(tempLocation, tempRotation, tempScale, mat);
			location = tempScale;
