...
// TODO: Rewrite
	...
	#define LE_WARNING_MANY(conditionExpr, ...)                                                                  \
		{                                                                                                        \
			...
			::LimeEngine::Assertions::Warning(condition, ::LimeEngine::LoggerManager::GetLogger(), __VA_ARGS__); \
	...
	#define LE_ASSERT_MANY(conditionExpr, ...)                                                                  \
		{                                                                                                       \
			bool condition = !!(conditionExpr);                                                                 \
			::LimeEngine::Assertions::Assert(condition, ::LimeEngine::LoggerManager::GetLogger(), __VA_ARGS__); \
			if (!condition && Debugger::IsDebuggerAttached()) LE_DEBUGBREAK();                                  \
		}                                                                                                       \
		LE_REQUIRE_SEMICOLON
