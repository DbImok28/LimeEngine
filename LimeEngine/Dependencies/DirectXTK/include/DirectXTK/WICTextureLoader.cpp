...
namespace
{
	...
	DXGI_FORMAT WICToDXGI(const GUID& guid) noexcept
	{
		...
		{
			if (memcmp(&g_WICFormats[i].wic, &guid, sizeof(GUID)) == 0) return g_WICFormats[i].format;
		}

#if (_WIN32_WINNT >= _WIN32_WINNT_WIN8) || defined(_WIN7_PLATFORM_UPDATE)
		if (g_WIC2)
		{
			if (memcmp(&GUID_WICPixelFormat96bppRGBFloat, &guid, sizeof(GUID)) == 0) return DXGI_FORMAT_R32G32B32_FLOAT;
		}
#endif
	...
	size_t WICBitsPerPixel(REFGUID targetGuid) noexcept
	{
		...
		if (FAILED(pWIC->CreateComponentInfo(targetGuid, cinfo.GetAddressOf()))) return 0;
		...
		if (FAILED(cinfo.As(&pfinfo))) return 0;
		...
		_Outptr_opt_ ID3D11ShaderResourceView** textureView) noexcept
	{
		...
		if (maxsize > UINT32_MAX) return E_INVALIDARG;
		...
		if (format == DXGI_FORMAT_UNKNOWN)
		{
			...
			{
				DebugTrace(
					"ERROR: WICTextureLoader does not support all DXGI formats (WIC GUID {%8.8lX-%4.4X-%4.4X-%2.2X%2.2X-%2.2X%2.2X%2.2X%2.2X%2.2X%2.2X}). Consider using "
					"DirectXTex.\n",
					pixelFormat.Data1,
					pixelFormat.Data2,
					pixelFormat.Data3,
		...
		else if (!(loadFlags & WIC_LOADER_IGNORE_SRGB))
		{
			...
			if (SUCCEEDED(frame->GetMetadataQueryReader(metareader.GetAddressOf())))
			{
				...
				if (SUCCEEDED(metareader->GetContainerFormat(&containerFormat)))
				{
					...
					if (sRGB) format = LoaderHelpers::MakeSRGB(format);
				}
			}
		}
		...
		if (rowBytes > UINT32_MAX || numBytes > UINT32_MAX) return HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW);
...
_Use_decl_annotations_ HRESULT DirectX::CreateWICTextureFromMemory(
	ID3D11Device* d3dDevice, const uint8_t* wicData, size_t wicDataSize, ID3D11Resource** texture, ID3D11ShaderResourceView** textureView, size_t maxsize) noexcept
{
	return CreateWICTextureFromMemoryEx(d3dDevice, wicData, wicDataSize, maxsize, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, WIC_LOADER_DEFAULT, texture, textureView);
}
		...
		size_t maxsize) noexcept
{
	return CreateWICTextureFromMemoryEx(
		d3dDevice, d3dContext, wicData, wicDataSize, maxsize, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, WIC_LOADER_DEFAULT, texture, textureView);
}
	...
	ID3D11ShaderResourceView** textureView) noexcept
{
	...
	if (!wicDataSize) return E_FAIL;

	if (wicDataSize > UINT32_MAX) return HRESULT_FROM_WIN32(ERROR_FILE_TOO_LARGE);
...
DirectX::CreateWICTextureFromFile(ID3D11Device* d3dDevice, const wchar_t* fileName, ID3D11Resource** texture, ID3D11ShaderResourceView** textureView, size_t maxsize) noexcept
{
	return CreateWICTextureFromFileEx(d3dDevice, fileName, maxsize, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, WIC_LOADER_DEFAULT, texture, textureView);
}
