...
namespace DirectX
{
	...
	{
	public:
		MapGuard(_In_ ID3D11DeviceContext* context, _In_ ID3D11Resource* resource, _In_ unsigned int subresource, _In_ D3D11_MAP mapType, _In_ unsigned int mapFlags) noexcept(
			false) :
			mContext(context),
			mResource(resource), mSubresource(subresource)
		{
...
// Helper sets a D3D resource name string (used by PIX and debug layer leak reporting).
	...
	template <typename T>
	HRESULT CreateInputLayoutFromEffect(_In_ ID3D11Device* device, _In_ IEffect* effect, _COM_Outptr_ ID3D11InputLayout** pInputLayout) noexcept
	{
